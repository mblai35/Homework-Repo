...
smat <- data.frame(MC1@states, steadyStates(MC1)[1, ]*15)
colnames(smat) <- c('state', 'ss')
# Bar chart of population for each state. 
qplot(state, data = smat, geom = "bar", weight = ss, 
...
#plot(movesMatrix[, 1], movesMatrix[, 2], type = 'l', 
 #    ylim = c(1,5), ylab = "States", xlab = "Position")
movesMatrix <- data.frame(movesMatrix)
ggplot(movesMatrix, aes(TimeStep, State)) + geom_path()
...
#    ylim = c(1,5), ylab = "States", xlab = "Position")
movesMatrix <- data.frame(movesMatrix)
ggplot(movesMatrix, aes(TimeStep, State)) + geom_*()
